-- Подсветка спецификаторов форматирования (%d, %s, %v и т.д.) в Go-строках.
-- fmt.Sprintf("value=%d", 42) — %d будет выделен цветом.
return {
  "AstroNvim/astrocore",
  init = function()
    ...
    vim.api.nvim_create_autocmd("ColorScheme", {
      callback = function() vim.api.nvim_set_hl(0, "UserGoFormatSpec", { fg = "#d3869b", bold = true }) end,
    })
    ...
    local function highlight(bufnr)
      ...
      if not ok or not parser then
        vim.defer_fn(function() highlight(bufnr) end, 200)
        return
      end
      ...
      local ok2, query = pcall(vim.treesitter.query.parse, "go", [[(interpreted_string_literal) @string]])
    ...
    vim.api.nvim_create_autocmd("FileType", {
      ...
      callback = function(args)
        vim.defer_fn(function() highlight(args.buf) end, 50)
      end,
    })

    vim.api.nvim_create_autocmd({ "TextChanged", "TextChangedI" }, {
      callback = function()
        if vim.bo.filetype == "go" then highlight(vim.api.nvim_get_current_buf()) end
      end,
    })
